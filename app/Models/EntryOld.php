...
class EntryOld extends Model
{
    ...
    public static function totalShiftData($type = 1,$input_date= ""){
        ...
        if(Auth::user()->priv == 1){
            ...
            $last_hour_upi_total = Entry::where('type',$type)->where('user_session_id',Auth::user()->session_id)->where('deleted',0)->where('pay_type',2)->whereBetween('created_at', [date('Y-m-d H:00:00'), date("Y-m-d H:i:s")])->sum("paid_amount"); 
            $last_hour_upi_total += DB::table('e_entries')->where('type',$type)->where('user_session_id',Auth::user()->session_id)->where('pay_type',2)->whereBetween('created_at', [date('Y-m-d H:00:00'), date("Y-m-d H:i:s")])->sum("paid_amount"); 
            
            $last_hour_cash_total = Entry::where('type',$type)->where('user_session_id',Auth::user()->session_id)->where('deleted',0)->where('pay_type',1)->whereBetween('created_at', [date('Y-m-d H:00:00'), date("Y-m-d H:i:s")])->sum("paid_amount");
            $last_hour_cash_total += DB::table('e_entries')->where('type',$type)->where('user_session_id',Auth::user()->session_id)->where('pay_type',1)->whereBetween('created_at', [date('Y-m-d H:00:00'), date("Y-m-d H:i:s")])->sum("paid_amount");
